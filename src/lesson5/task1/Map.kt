...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    val sumOfMap = mapA + mapB
    val resOfMap = mutableMapOf<String, String>()
    for ((key, value) in mapA) {
        if (value != sumOfMap[key])
            resOfMap[key] = "$value, ${sumOfMap[key]}"
    }
    return sumOfMap + resOfMap
}
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    val elm = word.map { "$it" }
    for (i in 0 until elm.size) {
        val element = elm[i].toLowerCase()
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    var pair = -1 to -1
    val list2 = list.mapIndexed { index, it -> it to index }.sortedBy { it.first }
